...
public class NoteRepository {
    ...
    public void insertNote(NoteEntity note) {
        note.userId = userId; note.id = UUID.randomUUID().toString();
        ...
        db.collection("users").document(userId) .collection("notes").document(note.id) .set(note);
    }
    ...
    public void archiveNote(NoteEntity note) {
        note.isArchived = true; updateNote(note);
    }
    public void trashNote(NoteEntity note) {
        note.isTrashed = true; updateNote(note);
    }
